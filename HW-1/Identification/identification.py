...
##########
# ### WARNING
# # If we plot the above instograms as subplots we notice that the normalized 
# # histogram misses some bars. This is due desktop formatting. For this reason we 
# # have written a small part of code in order to generate the two plots(normalized 
# # and matplotlib one) separately. In this case we have the two outputs in a 
# # correct way, without loosing bars. 
# # This problen could appear even for the other histograms plotted using subplot

# plt.figure(40,figsize = (15,15))
# num_bins_gray = 40
# hist_gray1, bin_gray1 = hist(img_gray.reshape(img_gray.size), num_bins_gray,(0,255))
# plt.bar((bin_gray1[0:-1] + bin_gray1[1:])/2, hist_gray1)
# plt.show()
# plt.figure(41,figsize = (15,15))
# hist_gray2, bin_gray2 = histogram_module.normalized_hist(img_gray, num_bins_gray)
# plt.bar((bin_gray2[0:-1] + bin_gray2[1:])/2, hist_gray2)
# plt.show()
##########
...
correct_order = np.array([int(i[9:-8]) for i in query_images])
dist_type = ['intersect', 'chi2', 'l2']
hist_type = ['rgb', 'rg', 'dxdy']
num_bins = [5, 10,20,30, 40]
Num_correct = pd.DataFrame(columns = ['hist_type', 'dist_type', 'num_bins', 'num of correct over 100'])
i = 0
for h in range(len(hist_type)):
    for d in range(len(dist_type)):
       for n in num_bins:
           [best_match, D] = match_module.find_best_match(model_images, query_images, dist_type[d], hist_type[h], n)
           best_match = np.array([int(i[9:-7]) for i in best_match])
           num_correct = sum(np.array(best_match) == correct_order)
           new_row = [hist_type[h], dist_type[d], n, num_correct]
           Num_correct.loc[i,:] = new_row
           i += 1
Num_correct = Num_correct.sort_values(by=['num of correct over 100'], ascending = False)
print(Num_correct)
