...
def plot_rpc(D, plot_color):
    ...
    for idt in range(len(d)):
        ...
        precision.append(tp/(tp+fp))  #tp/tp+fp where tp+fp = len(d)-(len(d)-idt+1)
        recall.append(tp/(num_images)) #num_images -> TruePositive + FalseNegative, or in alternative we can use simply tp+fn
    #print(tp)
