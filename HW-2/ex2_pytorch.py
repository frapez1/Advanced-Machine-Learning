...
if train:
    ...
    for epoch in range(num_epochs):
        for i, (images, labels) in enumerate(train_loader):
            ...
            #################################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            predicted=model(images) #pass the images to the model
            loss= criterion(predicted,labels) #to calculate the loss pass the output of the network along with the labels
            #oss: the criterion is a crossentropyloss which is useful when there is a training classification problem with C classess.
            #also https://pytorch.org/docs/master/nn.html#torch.nn.CrossEntropyLoss
            optimizer.zero_grad() # in pytorch is necessary to set the gradients to zero before starting 
            #to do backpropragration because Pytorch accumulates the gradients on subsequent backward passess. 
            loss.backward() # computes dloss/dx for every parameter x which has requires_grad=True. 
            #These are accumulated into x.grad for every parameter x.
            
            optimizer.step() #there isn't a step method so for this reason we call optimizer.step() in order to make the gradient descent step. 
        ...
        with torch.no_grad():
            ...
            for images, labels in val_loader:
                ...
                predicted = torch.max(output_mod,1)[1] #Returns a namedtuple (values, indices) where values is the maximum value of each row of the input tensor in the given dimension dim. And indices is the index location of each maximum value found (argmax).
